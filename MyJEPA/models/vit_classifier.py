"""ViT Classifier for finetuning on classification tasks."""
...
class ViTClassifier(nn.Module):
    """Vision Transformer classifier for ECG classification."""
    
    def __init__(self, encoder: VisionTransformer, config, use_sdp_kernel=True):
        super().__init__()
        self.config = config
        ...
        if config.attn_pooling:
            ...
            self.fc = nn.Linear(encoder.dim, config.num_classes, bias=config.bias)

        # Initialize new layers (excluding encoder)
