...
from models.predictor_grouped import GroupedPredictor
...
from models.vision_transformer_grouped import GroupedViT
...
class JEPA(nn.Module):
  def __init__(self, config: configs.pretrain.Config, momentum_schedule, use_sdp_kernel=True):
    ...
    if structure == 'factorized':
      ...
      self.encoder = GroupedViT(config, keep_registers=False, use_sdp_kernel=use_sdp_kernel)
      self.predictor = GroupedPredictor(config, use_sdp_kernel=use_sdp_kernel)
    else:  # 'standard'
  ...
  def forward(self, x, mask_encoder, mask_predictor):
    structure = getattr(self.config, 'structure', 'standard')
    ...
    with torch.no_grad():
      ...
      if is_2d_mask:
        ...
        if is_grouped:
          # For grouped: 4 groups x num_patches
          num_groups = 4
          num_time = self.config.num_patches
          h = apply_mask_2d(h, mask_pred_indices, num_groups, num_time)
        else:
          # For factorized: 12 leads x num_patches
        ...
        # 1D masking with grouped model: same time mask for all groups
        num_groups = 4
        h = apply_mask_factorized(h, mask_predictor, num_groups)
      else:
        # Standard 1D masking
