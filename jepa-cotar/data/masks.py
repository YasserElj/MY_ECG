...
class MaskCollatorGrouped:
  """
  Masking for Grouped Patch ECG (4 groups × 200 time patches = 800 tokens).
  
  Groups:
    Group 0: I, II, III (Limb leads)
    Group 1: aVR, aVL, aVF (Augmented leads)
    Group 2: V1, V2, V3 (Septal/Anterior)
    Group 3: V4, V5, V6 (Lateral precordial)
  
  Can operate in two modes:
    - '1d': Same time mask for all groups (like standard 1D masking)
    - '2d': Different masks per group (2D rectangular blocks in group × time space)
  """
  ...
  def __init__(self,
    ...
    self.max_keep_ratio = max_keep_ratio
    # 2D params
    self.min_block_groups = min_block_groups
    self.max_block_groups = max_block_groups
    self.min_block_time = min_block_time
    self.max_block_time = max_block_time
  ...
  def __call__(self, batch):
    ...
    if self.mode == '1d':
      return self._call_1d(batch, batch_size, num_time_patches)
    else:
      return self._call_2d(batch, batch_size, num_time_patches)
  
  def _call_1d(self, batch, batch_size, num_time_patches):
    """
    1D masking: same time mask for all groups.
    Output: batch, mask_encoder (B, K), mask_predictor (B, M)
    where K = kept time patches, M = masked time patches
    
    These are time indices only - model applies to all groups.
    """
    keep_ratio = np.random.uniform(self.min_keep_ratio, self.max_keep_ratio)
    num_keep = math.ceil(keep_ratio * num_time_patches)
    
    mask_encoder, mask_predictor = [], []
    for _ in range(batch_size):
      mask = self._sample_1d_mask(num_keep, num_time_patches)
  ...
  def _call_2d(self, batch, batch_size, num_time_patches):
    """
    2D masking: different masks per group.
    Output: batch, (mask_encoder, lengths), (mask_predictor, lengths)
    where masks are (B, K, 2) with (group_idx, time_idx) pairs.
    """
    mask_encoder_list, mask_predictor_list = [], []
    
    for _ in range(batch_size):
      # Create 2D mask grid: (num_groups, num_time_patches)
      mask_2d = torch.ones(self.num_groups, num_time_patches)
      ...
      attempts = 0
      while current_mask_ratio < target_mask_ratio and attempts < 30:
        # Random block dimensions
        block_groups = np.random.randint(
          self.min_block_groups, 
          min(self.max_block_groups + 1, self.num_groups + 1))
        block_time = np.random.randint(
          self.min_block_time, 
          min(self.max_block_time + 1, num_time_patches + 1))
        
        # Random position
        group_start = np.random.randint(0, self.num_groups - block_groups + 1)
        time_start = np.random.randint(0, num_time_patches - block_time + 1)
        
        # Mask this block
        mask_2d[group_start:group_start+block_groups,
                time_start:time_start+block_time] = 0
        
        current_mask_ratio = 1.0 - mask_2d.mean().item()
        attempts += 1
      
      # Convert to (group, time) index pairs
      kept_indices = mask_2d.nonzero(as_tuple=False)
      masked_indices = (1 - mask_2d).nonzero(as_tuple=False)
  ...
  def _sample_1d_mask(self, num_keep, num_patches):
    """Sample a 1D block mask (same as standard MaskCollator)."""
    patch_intervals = [(0, num_patches)]
    num_mask = num_patches - num_keep
    total_mask_size = 0
