"""
LeJEPA-ECG Finetuning Script

Fine-tune a pre-trained LeJEPA encoder on PTB-XL classification tasks.
...
# ViT config keys to extract from LeJEPA checkpoint
VIT_CONFIG_KEYS = [
...
parser.add_argument('--encoder', required=True, help='path to LeJEPA checkpoint')
...
@dataclass
class EncoderConfig:
    """ViT encoder configuration extracted from LeJEPA checkpoint."""
...
def main():
    ...
    # Extract encoder weights from LeJEPA model
    # LeJEPA saves as 'encoder.*', we need to strip the prefix
    model_state_dict = {
    ...
    # PTB-XL stratified splits (fold 9 = val, fold 10 = test, 1-8 = train)
